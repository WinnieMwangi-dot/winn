...
# Adjust the path of your model file
model_path = "rf_model L.pkl"  # Path to your model file
...
try:
    ...
    if not hasattr(model, 'predict'):
        st.error("Loaded object is not a valid model. Ensure it's a scikit-learn model or one with a 'predict' method.")
        model = None  # Clear the model to prevent further errors
except FileNotFoundError:
    st.error(f"Model file not found. Please ensure the model file '{model_path}' exists.")
    model = None  # Clear the model to prevent further errors
except Exception as e:
    st.error(f"An error occurred while loading the model: {e}")
    model = None  # Clear the model to prevent further errors

# If model is loaded successfully, proceed with the rest of the app
if model:
    # App title
    ...
    st.error("Model loading failed. Cannot proceed with prediction.")
